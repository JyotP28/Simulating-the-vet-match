...
function App() {
  ...
  /**
   * RESTORED: Full Analytics Section
   */
      ...
      <div style={{ background: tokens.colors.white, padding: '32px', borderRadius: '24px', border: `1px solid ${tokens.colors.border}`, marginBottom: '40px', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px' }}>
          <div>
            ...
            <StatRow label="Total Applicants Matched" simVal={matchedApps.length.toLocaleString()} targetVal="1,753" />
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', padding: '40px 20px', fontFamily: tokens.fonts.body, maxWidth: '1280px', margin: '0 auto', color: tokens.colors.heading, backgroundColor: '#fcfcfd' }}>
      ...
      <div style={{ flex: 1 }}>
        <header style={{ marginBottom: '56px', textAlign: 'center' }}>
          ...
          <p style={{ color: tokens.colors.subtext, fontSize: '18px', fontWeight: '500' }}>Using summary data from 2026</p>
        </header>

        {/* --- ROW 1: Symmetrical Input Boxes (1fr 1fr with internal flex-grow) --- */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px', marginBottom: '32px', alignItems: 'stretch' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', background: tokens.colors.white, padding: '40px', borderRadius: '24px', border: `1px solid ${tokens.colors.border}`, boxShadow: '0 4px 6px -1px rgba(0,0,0,0.03)' }}>
            ...
            <div style={{ marginTop: 'auto', paddingTop: '24px' }}>
              <label style={{ fontWeight: '700', display: 'block', marginBottom: '12px', fontSize: '13px' }}>Academic Class Rank Standings</label>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px', marginBottom: '40px', alignItems: 'stretch' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', background: tokens.colors.white, padding: '40px', borderRadius: '24px', border: `1px solid ${tokens.colors.border}` }}>
            ...
            <input type="text" placeholder="Search by hospital..." style={{ width: '100%', padding: '16px', marginBottom: '24px', border: `1px solid ${tokens.colors.border}`, borderRadius: '12px', fontSize: '14px' }} onChange={e => setSearchTerm(e.target.value)} />
        ...
        <button onClick={executeSimulation} disabled={userRankList.length === 0 || isSimulating} style={{ width: '100%', padding: '24px', background: userRankList.length === 0 ? '#cbd5e1' : `linear-gradient(135deg, ${tokens.colors.primary} 0%, #059669 100%)`, color: 'white', border: 'none', borderRadius: '20px', fontSize: '20px', fontFamily: tokens.fonts.display, fontWeight: '800', cursor: 'pointer' }}>
          {isSimulating ? 'Processing Matching Engine...' : 'Run Simulation Match'}
        </button>
        ...
        {matchResults && (
          <div style={{ marginTop: '40px', padding: '40px', background: userMatchedProgram ? '#f0fdf4' : '#fef2f2', border: `2px solid ${userMatchedProgram ? tokens.colors.primary : '#f87171'}`, borderRadius: '28px', textAlign: 'center' }}>
            <h2 style={{ margin: '0 0 12px 0', fontFamily: tokens.fonts.display, fontSize: '26px' }}>Your Simulation Result</h2>
            {userMatchedProgram ? (
              <p style={{ margin: 0, fontSize: '20px', fontWeight: '600' }}>Success! Matched choice #{userRankIndex}: <br/> <span style={{fontSize: '28px', fontWeight: '800', display: 'block', marginTop: '12px'}}>{userMatchedProgram.title}</span></p>
            ) : (
              <p style={{ margin: 0, fontSize: '20px', fontWeight: '600' }}>No match found. Expand your rank list or adjust priorities.</p>
            )}
          </div>
        )}

        {/* DATA EXPLORER & FULL STATISTICS */}
        {matchResults && (
          <div style={{ marginTop: '80px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', background: tokens.colors.accent, padding: '6px', borderRadius: '14px' }}>
                <button onClick={() => setExplorerMode('programs')} style={{ padding: '12px 28px', background: explorerMode === 'programs' ? 'white' : 'transparent', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '700', cursor: 'pointer' }}>Programs</button>
                <button onClick={() => setExplorerMode('schools')} style={{ padding: '12px 28px', background: explorerMode === 'schools' ? 'white' : 'transparent', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '700', cursor: 'pointer' }}>Schools</button>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '24px', height: '680px' }}>
              <div style={{ background: 'white', border: `1px solid ${tokens.colors.border}`, borderRadius: '24px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ padding: '24px', borderBottom: `1px solid ${tokens.colors.border}` }}>
                  <input type="text" placeholder="Quick filter..." style={{ width: '100%', padding: '14px', borderRadius: '12px', border: `1px solid ${tokens.colors.border}`, boxSizing: 'border-box' }} onChange={e => setExplorerSearch(e.target.value)} />
                </div>
              ...
              <div style={{ background: 'white', border: `1px solid ${tokens.colors.border}`, borderRadius: '24px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ padding: '20px 24px', borderBottom: `1px solid ${tokens.colors.border}`, fontWeight: '800', fontSize: '11px', textTransform: 'uppercase', color: tokens.colors.subtext }}>Applicant Pool</div>
                <div style={{ overflowY: 'auto', flex: 1 }}>
                  ...
                  ) : <div style={{ textAlign: 'center', marginTop: '120px', color: tokens.colors.subtext }}>Select an item to view data.</div>}
                </div>
              </div>
                ...
                {viewedApplicant ? (
                  <>
                    ...
                    <div style={{ background: '#eff6ff', padding: '24px', borderRadius: '18px', marginBottom: '28px', border: '1px solid #bfdbfe' }}>
                      <strong style={{ fontSize: '11px', textTransform: 'uppercase', color: '#1e3a8a', display: 'block', marginBottom: '16px' }}>Evaluation Metrics</strong>
